...
model <- randomForest(formula = median_house_value ~ ., data=trainData, ntree=500, importance=TRUE)
...
res <- predict(model, newdata=data_test)
#res <- exp(res)
...
```{r}
 results <- data.frame(id=house_df_test_ids$id,median_house_value = res)
