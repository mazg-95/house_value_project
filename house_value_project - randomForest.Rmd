...
house_df_test <- cbind(OHE_dataframe, house_df_test) %>%
  select(-c("ocean_proximity"), contains("ocean_proximity.ISLAND"))
```
...
df <- house_df %>%
  select(-contains('ocean_proximity'))
...
cols <- colnames(house_df %>% select(-c("median_house_value"), -contains("ocean_proximity")))
cols2 <- colnames(house_df_test %>% select(-contains("ocean_proximity")))
...
trainData <- house_df  %>% select(-c("bedrooms_rooms_ratio", "bedrooms_per_household", "rooms_per_household", "pop_per_household", "bedrooms_pop_ratio", "extra_rooms_ratio"))
...
# model <-train(median_house_value ~ . - bedrooms_rooms_ratio - bedrooms_per_household - rooms_per_household - pop_per_household - extra_rooms_ratio, 
#           trainData,
#           method="rf",
...
# model <- randomForest(formula = median_house_value ~ ., data=trainData)
# 
# # model$results
# 
# plot(model)
...
```{r, Predict}
# 
checkRMSE <- function(model_id){
  model <- h2o.getModel(model_id)
  test.h2o <- as.h2o(house_df_test)
  model_perf <- h2o.performance(model = model, newdata = test.h2o)
  # RMSE of best model
  h2o.rmse(model_perf)
}
best_model_id <- grid_perf@model_ids[[which.min(sapply(grid_perf@model_ids, checkRMSE))]]
best_model <- h2o.getModel(best_model_id)

data_test <- as.h2o(house_df_test)
res <- predict(best_model, newdata=data_test)
#res <- exp(res)
...
```{r}
 ...
 results <- data.frame(id=house_df_test_ids$id,median_house_value = res$predict)
 write_csv(results, file = "results.csv", append = FALSE, col_names = TRUE)
```