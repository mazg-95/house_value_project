...
El proposito del siguiente notebook es analizar y crear un modelo de regresion para predecir el valor `median_house_value`. La data utilizada es de un censo aplicado en el estado de California. 
El Dataset esta dividido por bloque. Y contiene los siguientes atributos: 
- `Latitude`
- `Longitud`
- `housing_median_age`: Contiene la media del grupo de casas de las cuales se obtuvo información.
- `total_rooms`: Total de habitaciones registrados dentro del bloque.
- `total_bedroom`s: Total de dormitorios registrados dentro del bloque.
- `population`: Población total del bloque.
- `households`: Número total de hogares, un grupo de personas que residen dentro de una unidad de hogar, para un bloque.
...
- `ocean_proximity`: Ubicación de la casa respecto del océano/mar.
...
trimVar <- function(data, colname){
  IQR <- quantile(data[[colname]], 0.75) - quantile(data[[colname]], 0.25)
  LI <- mean(data[[colname]]) - 1.75*IQR
  LU <- mean(data[[colname]]) + 1.75*IQR
  mask <- data[colname] < LU & data[colname] > LI
  return(data[mask,])
}
...
index<-createDataPartition(df$id, p=0.99, list=FALSE)
...
rmse.rf_lasso
rf_lasso <- list( rmse=rmse.rf_lasso, name="RF 1")
...
rf_lasso_scaled <- list(rmse=rmse.rf_lasso_scaled, "RF 2 Scaled")
...
library(xgboost)
y <- "median_house_value"
x <- setdiff(names(house_df_scaled), y)
train.h2o <- as.h2o(house_df_scaled)
auto_ml <- h2o.automl(
  x = x, y = y, training_frame = train.h2o, nfolds = 5, 
...
data_test <- as.h2o(house_df_test_scaled)
pred_test <- as.data.frame(predict(auto_ml@leader, newdata=data_test))
rmse.automl <- RMSE(pred_test$predict, house_df_test$median_house_value)
rmse.automl

res_autom_ml <- list(rmse=rmse.automl, name="AutoML")

data_comp <- as.h2o(house_df_comp_scaled)
res <- predict(auto_ml@leader, newdata=data_comp)


res <- as.data.frame(res)
results <- data.frame(id=house_df_comp_ids$id,median_house_value = res$predict)
write_csv(results, file = "results-automl.csv", append = FALSE, col_names = TRUE)

```
```{r, Results}

# rbind(linear, regul, rf, rf_scaled, rf_lasso, rf_lasso_scaled, xgb, xgb_scaled,xgb_cv_scaled, svm_scaled, ensemble_1)
rbind(linear, regul, rf, rf_scaled, rf_lasso, rf_lasso_scaled,xgb_cv_scaled, svm_scaled, ensemble_1, ensemble_2)
